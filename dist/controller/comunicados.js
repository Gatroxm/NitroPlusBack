...
const getComunicados = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const query = `SELECT tb_comunicados.id, tb_comunicados.fecha, tb_comunicados.asunto, tb_comunicados.linkPDF FROM tb_comunicados LEFT JOIN (tb_pilotos_id_sim RIGHT JOIN tb_sim_plat_codplat ON tb_pilotos_id_sim.idSimCodPlataforma = tb_sim_plat_codplat.id) ON tb_comunicados.idSimulador = tb_sim_plat_codplat.idSimulador WHERE (((tb_pilotos_id_sim.idPiloto)=${id} Or (tb_pilotos_id_sim.idPiloto) Is Null) AND ((tb_comunicados.activo)=1 Or (tb_comunicados.activo) Is Null) AND ((tb_pilotos_id_sim.idEstado)=1 Or (tb_pilotos_id_sim.idEstado) Is Null) AND ((tb_comunicados.idSimulador)=1 Or (tb_comunicados.idSimulador) Is Null)) ORDER BY tb_comunicados.id DESC;`;
    try {
