...
const initModels = (sequelize = connection.default) => {
    const tb_asignacion_comisario = _tb_asignacion_comisario(sequelize, sequelize_1.DataTypes);
    const tb_calendario = _tb_calendario(sequelize, sequelize_1.DataTypes);
    const tb_cat_coches = _tb_cat_coches(sequelize, sequelize_1.DataTypes);
    const tb_cat_piloto = _tb_cat_piloto(sequelize, sequelize_1.DataTypes);
    const tb_coches = _tb_coches(sequelize, sequelize_1.DataTypes);
    const tb_coches_sim = _tb_coches_sim(sequelize, sequelize_1.DataTypes);
    const tb_cod_plataforma = _tb_cod_plataforma(sequelize, sequelize_1.DataTypes);
    const tb_cod_sim_preclasificatorio = _tb_cod_sim_preclasificatorio(sequelize, sequelize_1.DataTypes);
    const tb_codificacion_divisiones = _tb_codificacion_divisiones(sequelize, sequelize_1.DataTypes);
    const tb_codificacion_resultados = _tb_codificacion_resultados(sequelize, sequelize_1.DataTypes);
    const tb_conceptos_comisarios = _tb_conceptos_comisarios(sequelize, sequelize_1.DataTypes);
    const tb_contenido_grafico = _tb_contenido_grafico(sequelize, sequelize_1.DataTypes);
    const tb_descargos_involucrados = _tb_descargos_involucrados(sequelize, sequelize_1.DataTypes);
    const tb_divisiones = _tb_divisiones(sequelize, sequelize_1.DataTypes);
    const tb_divisiones_pilotos = _tb_divisiones_pilotos(sequelize, sequelize_1.DataTypes);
    const tb_equipos = _tb_equipos(sequelize, sequelize_1.DataTypes);
    const tb_equipos_sim = _tb_equipos_sim(sequelize, sequelize_1.DataTypes);
    const tb_estado_carrera = _tb_estado_carrera(sequelize, sequelize_1.DataTypes);
    const tb_estado_general = _tb_estado_general(sequelize, sequelize_1.DataTypes);
    const tb_estado_inscripcion = _tb_estado_inscripcion(sequelize, sequelize_1.DataTypes);
    const tb_estado_penalizaciones = _tb_estado_penalizaciones(sequelize, sequelize_1.DataTypes);
    const tb_estado_preclasificatorios = _tb_estado_preclasificatorios(sequelize, sequelize_1.DataTypes);
    const tb_estado_reportes = _tb_estado_reportes(sequelize, sequelize_1.DataTypes);
    const tb_estado_torneos = _tb_estado_torneos(sequelize, sequelize_1.DataTypes);
    const tb_etapas_calendario = _tb_etapas_calendario(sequelize, sequelize_1.DataTypes);
    const tb_eventos_preclasificatorio = _tb_eventos_preclasificatorio(sequelize, sequelize_1.DataTypes);
    const tb_evidencias_reportes = _tb_evidencias_reportes(sequelize, sequelize_1.DataTypes);
    const tb_formato_carrera = _tb_formato_carrera(sequelize, sequelize_1.DataTypes);
    const tb_gravedad_sancion = _tb_gravedad_sancion(sequelize, sequelize_1.DataTypes);
    const tb_grupo_coches_personalizado = _tb_grupo_coches_personalizado(sequelize, sequelize_1.DataTypes);
    const tb_grupo_equipos = _tb_grupo_equipos(sequelize, sequelize_1.DataTypes);
    const tb_ident_pos = _tb_ident_pos(sequelize, sequelize_1.DataTypes);
    const tb_ident_pos_sim_preq = _tb_ident_pos_sim_preq(sequelize, sequelize_1.DataTypes);
    const tb_incidentes_sim = _tb_incidentes_sim(sequelize, sequelize_1.DataTypes);
    const tb_info_preclasificatorios = _tb_info_preclasificatorios(sequelize, sequelize_1.DataTypes);
    const tb_inscripcion_preclasificatorio = _tb_inscripcion_preclasificatorio(sequelize, sequelize_1.DataTypes);
    const tb_inscripciones = _tb_inscripciones(sequelize, sequelize_1.DataTypes);
    const tb_involucrados_incidente_sim = _tb_involucrados_incidente_sim(sequelize, sequelize_1.DataTypes);
    const tb_involucrados_sanciones = _tb_involucrados_sanciones(sequelize, sequelize_1.DataTypes);
    const tb_ligas = _tb_ligas(sequelize, sequelize_1.DataTypes);
    const tb_marcas_coches = _tb_marcas_coches(sequelize, sequelize_1.DataTypes);
    const tb_marcas_mando = _tb_marcas_mando(sequelize, sequelize_1.DataTypes);
    const tb_neumaticos = _tb_neumaticos(sequelize, sequelize_1.DataTypes);
    const tb_neumaticos_sim = _tb_neumaticos_sim(sequelize, sequelize_1.DataTypes);
    const tb_nombre_grupo_coches_personalizado = _tb_nombre_grupo_coches_personalizado(sequelize, sequelize_1.DataTypes);
    const tb_nombre_grupo_equipos = _tb_nombre_grupo_equipos(sequelize, sequelize_1.DataTypes);
    const tb_opcion_preclasificatorios_torneo = _tb_opcion_preclasificatorios_torneo(sequelize, sequelize_1.DataTypes);
    const tb_opciones_respuesta = _tb_opciones_respuesta(sequelize, sequelize_1.DataTypes);
    const tb_paises = _tb_paises(sequelize, sequelize_1.DataTypes);
    const tb_parrilla_calendario = _tb_parrilla_calendario(sequelize, sequelize_1.DataTypes);
    const tb_penalizacion_sanciones = _tb_penalizacion_sanciones(sequelize, sequelize_1.DataTypes);
    const tb_pilotos = _tb_pilotos(sequelize, sequelize_1.DataTypes);
    const tb_pilotos_id_sim = _tb_pilotos_id_sim(sequelize, sequelize_1.DataTypes);
    const tb_pilotos_liga = _tb_pilotos_liga(sequelize, sequelize_1.DataTypes);
    const tb_pilotos_penalizados = _tb_pilotos_penalizados(sequelize, sequelize_1.DataTypes);
    const tb_pistas = _tb_pistas(sequelize, sequelize_1.DataTypes);
    const tb_pistas_sim = _tb_pistas_sim(sequelize, sequelize_1.DataTypes);
    const tb_pistas_variantes = _tb_pistas_variantes(sequelize, sequelize_1.DataTypes);
    const tb_plataforma = _tb_plataforma(sequelize, sequelize_1.DataTypes);
    const tb_pregunta_inscripcion = _tb_pregunta_inscripcion(sequelize, sequelize_1.DataTypes);
    const tb_puntos = _tb_puntos(sequelize, sequelize_1.DataTypes);
    const tb_reparto_puntos = _tb_reparto_puntos(sequelize, sequelize_1.DataTypes);
    const tb_repeticiones = _tb_repeticiones(sequelize, sequelize_1.DataTypes);
    const tb_reportes_comisarios = _tb_reportes_comisarios(sequelize, sequelize_1.DataTypes);
    const tb_resultados = _tb_resultados(sequelize, sequelize_1.DataTypes);
    const tb_resultados_preclasificatorio = _tb_resultados_preclasificatorio(sequelize, sequelize_1.DataTypes);
    const tb_resultados_sim = _tb_resultados_sim(sequelize, sequelize_1.DataTypes);
    const tb_rol_involucrados = _tb_rol_involucrados(sequelize, sequelize_1.DataTypes);
    const tb_roles = _tb_roles(sequelize, sequelize_1.DataTypes);
    const tb_roles_pilotos = _tb_roles_pilotos(sequelize, sequelize_1.DataTypes);
    const tb_sesiones = _tb_sesiones(sequelize, sequelize_1.DataTypes);
    const tb_sesiones_sim = _tb_sesiones_sim(sequelize, sequelize_1.DataTypes);
    const tb_sim_data_sesion = _tb_sim_data_sesion(sequelize, sequelize_1.DataTypes);
    const tb_sim_plat_codplat = _tb_sim_plat_codplat(sequelize, sequelize_1.DataTypes);
    const tb_sim_version = _tb_sim_version(sequelize, sequelize_1.DataTypes);
    const tb_simcodplat_torneo = _tb_simcodplat_torneo(sequelize, sequelize_1.DataTypes);
    const tb_simulador = _tb_simulador(sequelize, sequelize_1.DataTypes);
    const tb_sistema_puntos = _tb_sistema_puntos(sequelize, sequelize_1.DataTypes);
    const tb_tabla_sanciones = _tb_tabla_sanciones(sequelize, sequelize_1.DataTypes);
    const tb_tabla_sanciones_sim = _tb_tabla_sanciones_sim(sequelize, sequelize_1.DataTypes);
    const tb_tipo_mando = _tb_tipo_mando(sequelize, sequelize_1.DataTypes);
    const tb_tipo_penalizacion = _tb_tipo_penalizacion(sequelize, sequelize_1.DataTypes);
    const tb_tipo_piloto = _tb_tipo_piloto(sequelize, sequelize_1.DataTypes);
    const tb_tipo_torneo = _tb_tipo_torneo(sequelize, sequelize_1.DataTypes);
    const tb_torneos = _tb_torneos(sequelize, sequelize_1.DataTypes);
