...
export const getLicencias = async (req: Request, res: Response) => {
  const { id } = req.params;
  const query = `SELECT tb_pilotos_id_sim.idPiloto AS id, tb_sim_plat_codplat.idSimulador, tb_sim_plat_codplat.idEstado AS idEstado, tb_licencias_pilotos.id AS LicenciaNo, tb_categoria_elo.nombreCategoria, tb_categoria_elo.descripcion, tb_categoria_elo.comoObtenerla, tb_categoria_elo.linkImagen, CASE WHEN tb_categoria_elo.linkFicha IS NOT NULL THEN CONCAT(tb_categoria_elo.linkFicha, '?&id_piloto=', tb_pilotos_id_sim.idPiloto, '&id_licencia=', tb_categoria_elo.id) END AS LinkFicha, MAX(tb_pilotos_id_sim.idSimPiloto) AS idSimPiloto, tb_info_clasificatorios.id AS idClasificatorio, tb_info_clasificatorios.requiereSala, tb_info_clasificatorios.linkTiempos, 1 AS isVisible, 1 AS idEstadoElo, tb_tipo_licencias.nivelLicencia, CASE WHEN tb_tipo_licencias.nombre IS NULL THEN 'Sin Licencia' END AS TextoLicencia, CASE WHEN tb_elo_actual.valorELO IS NOT NULL THEN CONCAT('Elo: ', tb_elo_actual.valorELO) END AS TextoELO, CASE WHEN tb_participaciones.participaciones IS NOT NULL THEN CONCAT('Participaciones: ', tb_participaciones.participaciones) END AS TextoParticipaciones, CASE WHEN tb_participaciones.indicadorCumplimiento IS NOT NULL THEN CONCAT('% Cump: ', tb_participaciones.indicadorCumplimiento) END AS TextoIndCump, CASE WHEN tb_sr_actual.valorSR IS NOT NULL THEN CONCAT('SR: ', tb_sr_actual.valorSR) END AS TextoSR, tb_tipo_licencias.nombre AS nombreNivelLicencia, tb_licencias_pilotos.idCategoriaELO AS idEloLicencias, tb_categoria_elo.id AS idCatElo, tb_elo_actual.valorELO, tb_participaciones.participaciones, tb_participaciones.indicadorCumplimiento, tb_sr_actual.valorSR FROM tb_pilotos_id_sim INNER JOIN tb_sim_plat_codplat ON tb_pilotos_id_sim.idSimCodPlataforma = tb_sim_plat_codplat.id INNER JOIN tb_categoria_elo ON tb_sim_plat_codplat.idSimulador = tb_categoria_elo.idSimulador AND tb_categoria_elo.idEstado = 1 LEFT JOIN tb_licencias_pilotos ON tb_pilotos_id_sim.idPiloto = tb_licencias_pilotos.idPiloto AND tb_categoria_elo.id = tb_licencias_pilotos.idCategoriaELO AND tb_categoria_elo.idSimulador = tb_licencias_pilotos.idSimulador LEFT JOIN tb_tipo_licencias ON tb_licencias_pilotos.idLicencia = tb_tipo_licencias.id LEFT JOIN tb_elo_actual ON tb_pilotos_id_sim.idPiloto = tb_elo_actual.idPiloto AND tb_categoria_elo.id = tb_elo_actual.idCategoriaELO LEFT JOIN tb_participaciones ON tb_sim_plat_codplat.idSimulador = tb_participaciones.idSimulador AND tb_pilotos_id_sim.idPiloto = tb_participaciones.idPiloto LEFT JOIN tb_sr_actual ON tb_sim_plat_codplat.idSimulador = tb_sr_actual.idSimulador AND tb_pilotos_id_sim.idPiloto = tb_sr_actual.idPiloto LEFT JOIN tb_info_clasificatorios ON tb_categoria_elo.id = tb_info_clasificatorios.idCategoriaElo WHERE tb_sim_plat_codplat.idEstado = 1 AND tb_pilotos_id_sim.idPiloto = ${id} GROUP BY tb_pilotos_id_sim.idPiloto, tb_sim_plat_codplat.idSimulador, tb_sim_plat_codplat.idEstado, tb_licencias_pilotos.id, tb_categoria_elo.nombreCategoria, tb_categoria_elo.descripcion, tb_categoria_elo.comoObtenerla, tb_categoria_elo.linkImagen, tb_categoria_elo.linkFicha, tb_info_clasificatorios.id, tb_info_clasificatorios.requiereSala, tb_info_clasificatorios.linkTiempos, 1, 1, tb_tipo_licencias.nivelLicencia, tb_tipo_licencias.nombre, tb_licencias_pilotos.idCategoriaELO, tb_categoria_elo.id, tb_elo_actual.valorELO, tb_participaciones.participaciones, tb_participaciones.indicadorCumplimiento, tb_sr_actual.valorSR ORDER BY tb_licencias_pilotos.id;`;
  try {
